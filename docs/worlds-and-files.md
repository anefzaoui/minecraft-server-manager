...
- **Import zip** — one button, two zip shapes, auto-detected:
  ...
  - A **plain zip of jars** you collected yourself: each jar is identified (Modrinth hash match → CurseForge fingerprint → the jar's own metadata) and judged against the server — *fits / wrong loader / wrong MC version / already installed / unidentified* — so you pick what actually belongs before anything installs.
...
A whole zip can also **create a server**: in the wizard's *From modpack* tab, upload a custom zip and the pack manifest (or a majority vote across the identified jars) fills in the loader and Minecraft version.
