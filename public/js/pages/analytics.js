...
const CROWN_SVG = `<svg class="icon size-3.5 shrink-0 text-gold-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M11.562 3.266a.5.5 0 0 1 .876 0L15.39 8.87a1 1 0 0 0 1.516.294L21.183 5.5a.5.5 0 0 1 .798.519l-2.834 10.246a1 1 0 0 1-.956.734H5.81a1 1 0 0 1-.957-.734L2.02 6.02a.5.5 0 0 1 .798-.519l4.276 3.664a1 1 0 0 0 1.516-.294z"/><path d="M5 21h14"/></svg>`;

const BADGE = {
  chat: 'bg-diamond-400/15 text-diamond-400',
  join: 'bg-grass-500/15 text-grass-400',
  leave: 'bg-inset text-ink-soft',
  death: 'bg-redstone-500/15 text-redstone-400',
  pvp: 'bg-redstone-500/15 text-redstone-400',
  advancement: 'bg-gold-400/15 text-gold-400',
  ...
  function renderEvent(evt) {
    ...
    if (evt.type === 'death') {
      ...
      if (evt.target) {
        ...
        pvp.className = 'badge ml-1.5 bg-redstone-500/15 text-redstone-400';
        pvp.textContent = 'PvP';
