...
function init() {
  ...
  function setSourceTab(tab) {
    ...
    if (sourceTab === 'mods') {
      ...
      solverState.slugs = [];
      if (hadSelection) toast('Left the "From mods" tab, so the queued mods were cleared.', { kind: 'info' });
    }
    sourceTab = tab;
  ...
  searchInput.addEventListener('input', () => {
    ...
    timer = setTimeout(async () => {
      try {
        ...
        toast(err.message || 'The Modrinth search could not be completed. Please try again.', { kind: 'error' });
      }
    }, 300);
  });
