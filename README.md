...
| `TRUST_PROXY` / `COOKIE_SECURE` / `COOKIE_SAMESITE`                                      | - / - / `lax`              | Set when behind a TLS-terminating reverse proxy. `TRUST_PROXY` (hop count, `loopback`, or IP/CIDR list - a bare `true` is refused at boot as it trusts a spoofable `X-Forwarded-For`) is required for `req.ip` to see the real client, which the rate limiters key on. `COOKIE_SECURE` is `true`/`false`/`auto` (auto needs `TRUST_PROXY`, enforced at boot). `COOKIE_SAMESITE` is `lax`/`strict`/`none`; `none` requires `COOKIE_SECURE`. |
